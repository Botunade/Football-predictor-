...
async def test_multi_sport_input_parsing():
    ...
    with patch('full_data_pipeline.fetch_player_info') as mock_player, \
        ...
        mock_predict.return_value = {
            "h2h": {"match": "Lakers vs Celtics", "model_probability": 0.60, "implied_probability": 0.51, "value": 0.09, "outcome": "Home"},
            "btts": {}, "over_25": {}
        }
        ...
        assert any("Analyzing Lakers vs Celtics" in r for r in self_replies)
    ...
    with patch('full_data_pipeline.fetch_player_info') as mock_player, \
        ...
        mock_player.return_value = {"injury_index": 0.1, "fatigue_index": 0.2, "key_player_dependency": 0.5}
        mock_predict.return_value = {
            "h2h": {"match": "Rangers vs Penguins", "model_probability": 0.60, "implied_probability": 0.47, "value": 0.13, "outcome": "Home"},
            "btts": {}, "over_25": {}
        }
        ...
        assert any("Analyzing Rangers vs Penguins" in r for r in self_replies)
    ...
    await handle_manual_input(mock_update, mock_context := MagicMock())
    self_replies = [call.args[0] for call in mock_update.message.reply_text.call_args_list]
    assert any("Analyzing Nadal vs Federer" in r for r in self_replies)
